    ...
    <x-container>
        <div>
            <div class="sm:hidden">
                ...
                <select id="tabs" name="tabs" class="block w-full rounded-md border-neutral-300 focus:border-indigo-500 focus:ring-indigo-500" wire:model="activeTab">
            ...
            <div class="hidden sm:block">
                <nav class="flex space-x-4" aria-label="Tabs">
                    ...
                    <a href="#" class="rounded-md px-3 py-2 text-sm font-medium {{ $activeTab === 'my-account' ? 'bg-red-500 text-neutral-100' : 'text-neutral-500 hover:text-neutral-700' }}" wire:click.prevent="setActiveTab('my-account')">Mein Konto</a>
                    <a href="#" class="rounded-md px-3 py-2 text-sm font-medium {{ $activeTab === 'membership' ? 'bg-red-500 text-neutral-100' : 'text-neutral-500 hover:text-neutral-700' }}" wire:click.prevent="setActiveTab('membership')">Meine Clubmitgliedschaft</a>
                </nav>
            </div>
        </div>
                        ...
                        <dl class="divide-y divide-neutral-100">
                            ...
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-neutral-700 sm:col-span-2 sm:mt-0">
                                    {{ $membership->clubRate->name }},  {{ $membership->clubRate->category->name }} Vertrag</dd>
                            </div>
                            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
